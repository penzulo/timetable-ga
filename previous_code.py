        ...
        for dept in self._data.get_depts():
            for course in dept.get_courses():
                for _ in range(course.get_lectures_per_week()):
                    ...
                    professor = (
                        rd.choice(course.get_professors())
    ...
    def _select_tournament_population(self, pop):
        ...
        for _ in range(TOURNAMENT_SELECTION_SIZE):
            tournament_pop.get_schedules().append(
                pop.get_schedules()[rd.randrange(0, len(pop.get_schedules()))]
            )
...
def main():
    ...
    room_count = st.number_input(
        "Number of Rooms:", min_value=1, max_value=20, value=5, key="room_count"
    )
    for i in range(room_count):
        room_number = st.text_input(f"Room {i + 1} Number:", key=f"room_number_{i}")
        data.add_room(Room(room_number))

    lab_room_count = st.number_input(
        "Number of Lab Rooms:", min_value=1, max_value=20, value=5, key="lab_room_count"
    )
    for i in range(lab_room_count):
        lab_room_number = st.text_input(
            f"Lab Room {i + 1} Number:", key=f"lab_room_number_{i}"
        )
    ...
    dept_count = st.number_input(
        "Number of Departments:", min_value=1, max_value=5, value=2, key="dept_count"
    )
    for i in range(dept_count):
        dept_name = st.text_input(f"Department {i + 1} Name:", key=f"dept_name_{i}")
        course_count = st.number_input(
            f"Number of Courses in {dept_name}:",
            min_value=1,
            max_value=20,
            value=3,
        ...
        for j in range(course_count):
            course_name = st.text_input(
                f"Course {j + 1} Name in {dept_name}:", key=f"course_name_{i}_{j}"
            )
            ...
            courses.append(
                Course(
                    f"C{j + 1}", course_name, professors, course_lectures, course_labs
                )
            )
        data.add_dept(Department(dept_name, courses))

    panel_count = st.number_input(
        "Number of Panels:", min_value=1, max_value=15, value=2, key="panel_count"
    )
